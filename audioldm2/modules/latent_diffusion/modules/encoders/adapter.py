...
from audioldm2.latent_diffusion.modules.diffusionmodules.util import zero_module
...
# T2I-Adapter: Input conditions
class Adapter(nn.Module):
    def __init__(self, channels=[320, 640, 1280, 1280], nums_rb=3, cin=64, ksize=3, sk=False, use_conv=True):
        """
        T2I-Adapter: Input image conditions such as depth image, keypose image and so on.
        Args:
            channels (list, optional): the input dimension of different layer in Adapter module. Defaults to [320, 640, 1280, 1280].
            nums_rb (int, optional): the number of layer in a resnet block. Defaults to 3.
            cin (int, optional): dimension of hidden space(?). Defaults to 64.
            ksize (int, optional): size of kernel. Defaults to 3.
            sk (bool, optional): whether apply SKNet attention mechanism. Defaults to False.
            use_conv (bool, optional): whether add conv layers. Defaults to True.
        """
        super(Adapter, self).__init__()
        self.unshuffle = nn.PixelUnshuffle(4)
        self.channels = channels # channels 中的每个数字表示了 Adapter 模块中的不同层的通道数
        self.nums_rb = nums_rb
        self.body = []
        for i in range(len(channels)):
            for j in range(nums_rb):
                # 下采样操作通常在每个resnet block的第一个卷积层中进行
                if (i != 0) and (j == 0):
                    self.body.append(
