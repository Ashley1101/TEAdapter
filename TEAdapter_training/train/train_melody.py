...
def load_resume_state(opt):
    ...
    if opt.auto_resume:
        state_path = osp.join('experiments', f"{opt.name}_all", 'training_states')
...
def main():
    ...
    train_dataset = MelodyDataset(config=audioldm2_config, dataset_name=["musiccaps", "FMA-intro", "FMA-chorus", "FMA-outro"])
    ...
    experiments_root = osp.join('experiments', f"{opt.name}_all")
    ...
    if resume_state is None:
        ...
        log_file = osp.join(experiments_root, f"train_{opt.name}_all_{get_time_str()}.log")
