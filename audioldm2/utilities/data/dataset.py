...
class AudioDataset(Dataset):
    def __init__(
        ...
        if dataset_json_path is not None:
            ...
            for extra_cond_name in self.extra_cond_opt:
                assert extra_cond_name in self.config["data"]["extra_cond"].keys(), (
                    "The extra condition %s you specified is not present in the config. You can choose from %s"
                    % (split, self.config["data"]["extra_cond"].keys())
                )
            self.build_dataset()
    ...
    def feature_extraction(self, index):
            ...
            try:
                ...
                for extra_cond_name in self.extra_cond_opt:
                    ...
                    (
                      ...
                      waveform_,
                      random_start_,
                    ) = self.read_audio_file(extra_datum["wav"])
                    extra_datum_dict[extra_cond_name] = extra_datum
                extra_cond_log_mel_spec = {}
                extra_cond_waveform = {}
                for extra_cond_name in self.extra_cond_opt:
                    extra_cond_log_mel_spec[extra_cond_name]=extra_datum_dict[extra_cond_name][log_mel_spec_]
                    extra_cond_waveform[extra_cond_name]=torch.FloatTensor(extra_datum_dict[extra_cond_name][waveform_])
        ...
        extra_fname = {}
        for extra_cond_name in self.extra_cond_opt:
            extra_fname[extra_cond_name]=extra_datum[extra_cond_name]["wav"]
    ...
    def build_dataset(self):
        ...
        for extra_cond_name in self.extra_cond_opt:
            ...
            extra_data_json = load_json(
                self.get_dataset_metadata_path(self.dataset_name, key=extra_cond_name)
            )
            extra_data_json = self._relative_path_to_absolute_path(
                data_json, self.dataset_name
            )
            self.extra_cond_data[extra_cond_name] = data_json["data"]
