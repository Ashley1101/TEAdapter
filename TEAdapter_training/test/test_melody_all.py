...
def parsr_args():
    ...
    parser.add_argument(
        '--adapter_ckpt',
        type=str,
        default='experiments/melody_all/models/model_ad_45000.pth',
    ...
    parser.add_argument(
        ...
        type=str,
        # default='experiments/melody/models/model_ad_11000.pth', 
        default='experiments/melody_outro/models/model_ad_20000.pth',
        help='path to checkpoint of adapter',
    )
    ...
    parser.add_argument(
        "--cond_filepath",
        type=str,
        default='data/musiccaps/audios/UVY68vFiApA.mp3',
...
def main():
    ...
    if opt.test_on_dataset:
        ...
        test_dataset = AudioDataset(extra_cond_opt=['melody', 'chord'],
        ...
        # Generate samples with extra conditional adapter    
            ...
            if opt.save_path is None:
                if len(which_cond)>1:
                    opt.save_path = f'outputs/test_0.4c_0.6m_all+instrument'
                else:
                    opt.save_path = f'outputs/test-{which_cond}_all'
            os.makedirs(opt.save_path, exist_ok=True) 
            CLAP_scores = {}
            ...
            with torch.no_grad():
                ...
                for _, data in enumerate(test_dataloader):
                    ...
                    if len(which_cond) > 1:
                        ...
                        cond = data['extra_cond_log_mel_spec'][which_cond[0]].unsqueeze(1).to(device)
        ...
        if opt.original_generation:
            ...
            output_file = os.path.join(opt.clap_score_path, f'clap_{which_cond}_all.json')
    ...
    # generate several samples
        ...
        save_wave(waveform, savepath=opt.save_path, name=track_id)
